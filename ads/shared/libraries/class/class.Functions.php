...
//        $x = (file_exists($patch)) ? getimagesize($patch) : null;
    ...
    public function get_them($table = '', $type = '', $id = '', $title_select = 'Chọn danh mục', $class_select = 'select-category')
    {
        ...
        $str = '<select name="data[' . $id . ']" class="form-control select2"><option value="0">' . $title_select . '</option>';
        foreach ($rows as $v) {
    ...
    public function get_posts($sluglang, $truyvan, $class, $thumb = '', $desc = 1, $day = 0, $view = 0)
    {
        ...
        foreach ($truyvan as $k => $v) { ?>
            <div class="<?= $class ?>">
                <p class="img_post zoom_hinh"><a href="<?= $v[$sluglang] ?>"><img
    ...
    public function get_posts_slick($truyvan, $class, $thumb, $desc = 1, $day = 0, $view = 0)
    {
        ...
        foreach ($truyvan as $k => $v) { ?>
            <div>
                <div class="<?= $class ?>">
                    <p class="img_post zoom_hinh"><a href="<?= $v[$sluglang] ?>"><img class="img-fluid no_lazy"
                                                                                      data-lazy="<?= $thumb ?><?= UPLOAD_NEWS_L . toWebp($v['photo']); ?>"
                                                                                      alt="<?= $v['ten'] ?>"></a></p>
                    <h3><a class="name_post catchuoi2" href="<?= $v[$sluglang] ?>"><?= $v['ten'] ?></a></h3>
                    <?php if ($day > 0) { ?><p class="day_post"><?= ngaydang ?>
                        : <?= date("d/m/Y h:i A", $v['ngaytao']) ?></p><?php } ?>
    ...
    public function get_posts_no($truyvan, $class, $thumb = '', $desc = 1, $day = 0, $view = 0)
    {
        ...
        foreach ($truyvan as $k => $v) { ?>
            <div class="<?= $class ?>">
                <?php if ($v['photo'] != '') { ?>
                    <p class="img_post"><img class="img-fluid no_lazy"
                                             src="<?= $thumb ?><?= UPLOAD_NEWS_L . toWebp($v['photo']); ?>"
                                             alt="<?= $v['ten'] ?>"></a></p>
                <?php } ?>
    ...
    public function lay_video($truyvan, $thumb)
    {
        ...
        foreach ($truyvan as $k => $v) { ?>
            <div class="item"><a data-fancybox="video" href="<?= $v['link_video'] ?>" title="<?= $v['ten'] ?>">
                    <p class="img_sp zoom_hinh"><img
                                src="https://img.youtube.com/vi/<?= $this->getYoutube($v['link_video']); ?>/0.jpg"
                                alt="<?= $v['ten'] ?>"/></a></p>
                <h3 class="name_sp"><a data-fancybox="video2" href="<?= $v['link_video'] ?>"
        ...
        foreach ($truyvan as $k => $v) { ?>
            ...
            <div class="item">
                <a data-fancybox="album<?= $k ?>" href="<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" title="<?= $v['ten'] ?>">
                    <p class="img_sp zoom_hinh"><img class="img-fluid" src="<?= $dongdau . UPLOAD_PRODUCT_L . toWebp($v['photo']) ?>"
                                                     alt="<?= $v['ten'] ?>"></p>
                    <h3 class="name_sp catchuoi2"><span><?= $v['ten'] ?></span></h3></a>
                <?php foreach ($hinhanhsp as $k2 => $v2) { ?>
                    <a data-fancybox="album<?= $k ?>" href="<?= UPLOAD_PRODUCT_L . $v2['photo'] ?>"
                       title="<?= $v2['ten'] ?>"></a>
                <?php } ?>
            </div>
        <?php }
    ...
    public function get_product($truyvan)
    {
        ...
        foreach ($truyvan as $k => $v) { ?>
            <div class="item">
                <div class="img_sp zoom_hinh"><a href="<?= $v[$sluglang] ?>" title="<?= $v['ten'] ?>"><img
                                src="<?= $dongdau . UPLOAD_PRODUCT_L . toWebp($v['photo']) ?>"
                                alt="<?= $v['ten'] ?>"></a>
                ...
                <h3 class="name_sp catchuoi2"><a href="<?= $v[$sluglang] ?>"
                                                 title="<?= $v['ten'] ?>"><?= $v['ten'] ?></a></h3>
                ...
                <p class="gia_sp">
                    ...
                    <span class="gia <?php if ($v['giamoi'] > 0) echo 'giacu' ?>"><?php if ($v['gia'] > 0) echo $func->format_money($v['gia']); else echo lienhe; ?></span>
                </p>
            ...
            <div>
                <div class="item item_i">
                    <div class="img_sp zoom_hinh">
                        <a href="<?= $v[$sluglang] ?>" title="<?= $v['ten'] ?>"><img class="img-fluid no_lazy"
                                                                                     data-lazy="<?= $dongdau . UPLOAD_PRODUCT_L . toWebp($v['photo']) ?>"
                                                                                     alt="<?= $v['ten'] ?>"></a>
                        <span class="cart-buy addcart transition " data-id="<?= $v['id'] ?>"
                              data-action="buynow"></span>
                        <?php if ($v['soluong'] <= 0 || $v['hethang']) echo '<p class="hethang"><span class="hethang-text">'.getLang('hethang').'</span></p>'; ?>
                    </div>
                    <h3 class="name_sp catchuoi2"><a href="<?= $v[$sluglang] ?>"
                                                     title="<?= $v['ten'] ?>"><?= $v['ten'] ?></a></h3>
