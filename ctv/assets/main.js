var resultsForm = document.getElementById("form-results");
...
//2
var resultsForm2 = document.getElementById("form-results-2");
var resultValidation2 = new te.Validation(resultsForm2, {
  customErrorMessages: {
    contains: "Dữ liệu không hợp lệ!",
    input: "Dữ liệu không hợp lệ!",
  },
...
resultsForm2.addEventListener("valid.te.validation", (e) => {
  const results = [];
  resultsForm2.querySelectorAll("input").forEach((input) => {
    results.push({ [input.name]: input.value });
  });
});
