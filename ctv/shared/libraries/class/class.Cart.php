<?php
	class Cart
	{
		...
		private function discount($code ='register'){
			return false;

			$file = 'EGiftVoucherSystem';

			$file = SHAREDLIBRARIES."class/class.".str_replace("\\","/",trim($file,'\\')).'.php';
			if(file_exists($file)) require_once $file;

			$uid_Voucher = new EGiftVoucherUser($this->d);
			$uid_Voucher->rate($code);
